...
public class Plateau {
	...
	* */
	...
	public void initTableau(){
		...
		// Ajout des pièces présentes dans le fichier JSON
		JSON_parser parser = new JSON_parser("config/config.json");
		Piece_json[] tabPiece_jsons = parser.getTabPieces_Json();
		for (int i = 0; i < tabPiece_jsons.length; i++) {
			add(tabPiece_jsons[i], tabPiece_jsons[i].getCase_depart());
		}
	...
	public void GM(){
		...
		while (!is_finished) {
			...
			int colone_A = index_A/10;
			int ligne_A = index_A-index_A*10;
			


			
			// On vérifie que si le joueur prend le roi alors la partie est terminée
			if(plateau[ligne_A][colone_A].getUNICODE() == 0x2654 || plateau[ligne_A][colone_A].getUNICODE() == 0x265A){
				is_finished = true;
			}
